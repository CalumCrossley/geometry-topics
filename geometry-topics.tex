...
\begin{equation*}
    h^r + \pi^*c_1(E)h^{r-1} + \cdots + \pi^*c_{r-1}(E)h + \pi^*c_r(E) = 0.
        \tag{$*$}
\end{equation*}
\textbf{Fact:} $H*(\P(E))=H^*(X)\oplus H^*(X)h\oplus\cdots\oplus H^*(X)h^{r-1}$
as a vector space. So $h^r$ can be uniquely written in this basis as a linear
combination of $1,h,\ldots,h^{r-1}$, and we can define $c_i(E)$ via ($*$).
...
\end{equation*}
for any ad-invariant ($p(N^{-1}MN)=p(M)$) polynomial function $p$ on
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.5]{chern_tangent}
\end{figure}
...
\begin{proposition}
    ...
    category. (Semi-simple means all subobjects are summands.)

    The category of semi-pure Hodge structures (direct sums of pure Hodge
    structures of different weights) has a natural ``tensor product'', making it
    a Tannakian category. (With a notion of dual, e.t.c.)
\end{proposition}
...
\end{equation*}
To comparewith de Rham cohomology, let $\omega$ be a $C^\infty$ differential
...
\begin{equation*}
    E_2^{p,q} = H^p(X,\Omega^{q,\an}_X)
        \Rightarrow H^n(X,\underline\C) = H^n(X;\C)_\C
\end{equation*}
