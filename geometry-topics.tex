...
\end{equation*}
We call the resulting line bundle $\calO(n)$ with $c_1=n$.

\paragraph{Tautological bundle.}
When $n=-1$ we get the \emph{tautological bundle} $\calO(-1)\to\P^1$. Over $\R$
this is the M\"obius bundle on $\RP^1\cong S^1$:
% TODO: diagram

\paragraph{Tautological bundle over $\C$.}
Over $\C$ we also see that $\calO(-1)$ (defined as above with transition
function $\frac{1}{x}$) is the tautological bundle $\calO(-1)$ over $\P^1$.
% TODO: diagram

\paragraph{Zeros of sections.}
The $\calO(n)$ line bundle over $\P^1$ was defined with transition function
$x^{-n}$, gluing the section 1 over $\C_x$ to $x^{-n}=y^n$ over $\C_y$. This
therefore defines a \emph{global} holomorphic section of $\calO(n)$ when
...
the sections since $\Gamma(\calO(n))=\Sym^n(\C^2)^*$.) Again these all have $n$
zeros.
\begin{exercise}
...
multiplicity) of \emph{any} section of $\calO(n)$. In other words, $c_1(L)$ is
...
\paragraph{Whitney sum formula I.}
...
\end{equation*}
In particular, for line bundles we get
$c_1(L_1\oplus L_2)=c_1(L_1)\cup c_1(L_2)$, and
\begin{align*}
    c_1(L_1\oplus L_2)
...
\paragraph{Whitney sum formula II.}
...
\begin{align*}
    \bigl[\bigl(&(e_1\wedge\cdots\wedge e_r)
...
Knowing (or defining!) $c_1(\O_{\P^n}(1))=[\P^{n-1}]$, the Whitney sum formula
and functoriality is then enough to completely determine all Chern classes on
...
\begin{itemize}
    \item All rank $r$ bundles on $X$ are pullbacks $f^*Q$ of the
        \emph{universal bundle on the classifying space} $Q\to\BGL(r,\C)$ by a
        map $f:X\to\BGL(r,\C)$. (So we only need to define $c_i$ on one
        classifying space.)
...
$\underline{\Gamma(E)}$:
\begin{equation*}
    \underline{\Gamma(E)} \xrightarrow{\ev} E\to 0.
\end{equation*}
(Or take a sufficiently large subbundle
$\underline{\C^N}\subseteq\underline{\C^\infty}=\underline{\Gamma(E)}$,
$N\gg0$.)

Therefore it defines a map to the Grassmannian
\begin{equation*}
    f
\end{equation*}
