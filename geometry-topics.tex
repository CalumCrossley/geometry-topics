...
\emph{intersection multiplicity}
\begin{equation*}
...
Suppose $\dim M=2m$. We get a pairing on $H_m(M)/\text{tor}$, and this is
...
\begin{definition}
    ...
    point a multi-linear alternating form
    $\alpha_p:\overbrace{T_pM\otimes\cdots\otimes T_pM}^{\text{$k$ times}}\to\R$.
\end{definition}

The space $\Omega^k(M)$ of $k$-forms has extra structure:
\begin{itemize}
    \item Wedge product: $\Omega^k(M)\otimes\Omega^l(M)\to\Omega^{k+l}(M)$.
    \item Exterior derivative: $\Omega^k(M)\to\Omega^{k+1}(M)$.
\end{itemize}
satisfying
\begin{itemize}
...
\begin{example}
    What is $H^1_\dR(\R)$? $\Omega^1(\R)=\{fdx:f:\R\to\R\}$ all are closed from
    above. Exact 1-forms are given by $dF=F'(x)dx$ for $F:\R\to\R$. Such an $F$
    always exists by the FTC, with
    \begin{equation*}
        F(x) = \int_0^xf(t)dt.
    \end{equation*}
    Hence $H^1_\dR(\R)=0$.
...
\begin{exercise}
    ...
    \end{equation*}
    (You may use a theorem of Thom, which states that up to integer multiples
    singular homology classes of a manifold may be represented by compact
    submanifolds.)
\end{exercise}
